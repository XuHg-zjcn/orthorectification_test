...
if __name__ == '__main__':
    ...
    for fid, name, poly, cpoint in db.get_intersect(fidA):
        ...
        for iidD, im in match_other(db, imgA_, H_B_to_Ap, iB, lst, extCoef=0, extMin=10):
            ...
            poly_A_in_D = im.get_poly_A_in_B()
            print('match insert to db', iidA, iidD)
            print(H_D_to_A)
            print(poly_D_in_A)
            db.insert_replace_match(iidA, iidD, H_D_to_A, poly_D_in_A.wkt, n_match)
            db.insert_replace_match(iidD, iidA, H_D_to_A.inv(), poly_A_in_D.wkt, n_match)
            db.commit()
        print('==========================')
    db.close()